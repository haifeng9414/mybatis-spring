...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
    ...
    //用于创建ObjectWrapper，ObjectWrapper能够在一个对象上执行getter和setter等操作
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
        ...
        //获取configuration，未设置则使用MyBatis的默认configuration配置，即直接new Configuration()
        ...
        if (this.configuration != null) {
            ...
            //这里直接过去configuration而不是通过xmlConfigBuilder.parse()获取是因为在解析configuration之前需要设置其他的属性即下面的多个if，在
            //设置这些属性后调用xmlConfigBuilder.parse对指定的mybatis-config.xml文件进行解析
